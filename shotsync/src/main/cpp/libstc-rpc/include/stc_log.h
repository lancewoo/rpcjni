...
//	#define RPC_PRINTF(x, a...) ALOGI(x, ##a)
	#define RPC_PRINTF(x, a...) __android_log_print(ANDROID_LOG_INFO, LOG_TAG, x, ##a)
#else
...
#define RPC_PUTS(x) do {\
	RPC_PRINTF("%s", x);\
		...
		#define RPC_ERROR(x, a...) do { \
				__android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "Error at [%s:%d] " x, __func__, __LINE__, ##a);\
			} while (0)
	#else
		#define RPC_ERROR(x, a...) do {\
...
//		#define RPC_DEBUG(x, a...) ALOGD(x, ##a)
		#define RPC_DEBUG(x, a...) __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, x, ##a)
	#else
...
//		#define RPC_INFO(x, a...) ALOGI(x, ##a)
		#define RPC_INFO(x, a...) __android_log_print(ANDROID_LOG_INFO, LOG_TAG, x, ##a)
	#else
